...
ngx_http_dyups_body_handler(ngx_http_request_t *r)
{
    ...
    // 请求路径是 /upstream/upstream名称 只能有两个
    if (res->nelts != 2) {
        ngx_str_set(&rv, "not support this interface");
        status = NGX_HTTP_NOT_FOUND;
        goto finish;
...
// 读消息队列中的数据，用于更新每个worker中的upstream配置
static void
ngx_http_dyups_read_msg_locked(ngx_event_t *ev)
{
    ...
    {
        msg = ngx_queue_data(q, ngx_dyups_msg_t, queue);
        // msg count等于进程数量，说明所有worker都读取了该数据，可以删除了
        ...
        msg->pid[i] = ngx_pid;
        // 每个worker读取一次消息都递增该计数器，等于worker数量时就可以删除了
        msg->count++;
